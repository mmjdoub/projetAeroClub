...
77 MAJ-NUMETYPE PIC X(2).
77 COUNT-TYPE PIC 99.
...
SCREEN SECTION.
  ...
  01  SCREEN-ACCUEIL.
      ...
      05 LINE 21 COL 01 VALUE "RECOMMENCER (R) OU REVENIR AU MENU (M) OU QUITTER (Q) :".
  ...
  01  SCREEN-MAJ.
	  ...
	  05 LINE 14 COL 01 VALUE "Entrez une nouvelle habilitation(type d'avion) : "
	  05 LINE 14 COL 50 PIC 9(3) FROM WS-NUMETYPE. 
	  05 LINE 14 COL 53 PIC 9(3) TO MAJ-NUMETYPE.
	  ********************date pilotage
...
CREATION.
	...
	IF CHOIXVALIDATION = 'O' OR CHOIXVALIDATION = 'o' 
		...
		EXEC SQL 
			SELECT COUNT(NUMETYPE) INTO :COUNT-TYPE
			FROM TYPES 
			WHERE NUMETYPE = WS-NUMETYPE
		END-EXEC.
		
		IF COUNT-TYPE = 0
			MOVE "Le type d'avion spécifié n'est pas connu." TO ERREURS
			PERFORM EVALUATE-CHOIX
		END-IF.
		
		
		EXEC SQL
			INSERT INTO PILOTAGE(NUMETYPE,NUMPIL,DATEPILOTAGE)
			VALUES(:WS-NUMETYPE,:WS-NUMPIL,:WS-DATEPILOTAGE)
		END-EXEC
...
MAJ.
	...
	EXEC SQL
		SELECT NUMPIL
		INTO :WS-NUMPIL FROM PILOTES
		WHERE NUMPIL = NUMPILUSER
	END-EXEC.
	IF WS-NUMPIL = 0 
		MOVE "Numéro de pilote inconnu." TO ERREURS
		PERFORM MAJ
	ELSE 
		EXEC SQL 
		...
		ACCEPT MAJ-NUMETYPE PIC X(2) LINE 14 COL 25
		...
		IF CHOIXVALIDATION = 'O' OR CHOIXVALIDATION = 'o' 
			...
			END-IF
		ELSE 
			MOVE "Les informations n'ont pas été modifiées." TO ERREURS		
			DISPLAY DELETE-SCREEN
			ACCEPT WDATE FROM DATE
			DISPLAY SCREEN-ACCUEIL
			ACCEPT SCREEN-ACCUEIL
